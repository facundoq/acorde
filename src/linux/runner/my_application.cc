...
static void my_application_activate(GApplication* application) {
  ...
  if (use_header_bar) {
    ...
    gtk_header_bar_set_title(header_bar, "acorde");
    gtk_header_bar_set_show_close_button(header_bar, TRUE);
    gtk_window_set_titlebar(window, GTK_WIDGET(header_bar));
  } else {
    gtk_window_set_title(window, "acorde");
  }

  gtk_window_set_default_size(window, 1280, 720);

  // Set window icon programmatically from assets
  ...
  if (exe_dir != nullptr) {
    ...
    g_autofree gchar* icon_path = g_build_filename(exe_dir_path, "data", "flutter_assets", "assets", "images", "icon.png", nullptr);
