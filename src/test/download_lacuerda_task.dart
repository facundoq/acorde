...
const String defaultSongsPath = '/home/facundoq/dev/acorde/assets/default_songs.json';
...
void main() {
  test('LaCuerda Scraper', () async {
    ...
    if (FileSystemEntity.typeSync(statusCsvPath) != FileSystemEntityType.notFound) {
    ...
    for (final artistQuery in targetArtists) {
      ...
      try {
        ...
        if (artistUrl != null) {
          ...
          print('No specific artist URL found, using ${songsList.length} direct song search results.');
        }

        final searchUrl = 'https://acordes.lacuerda.net/busca.php?exp=${Uri.encodeComponent(artistQuery)}';
        final searchHtml = htmlCache[searchUrl] ?? htmlCache[artistUrl] ?? '';
        String? artistDir;
        if (searchHtml.isNotEmpty) {
          final videoMatch = RegExp(r'videos\.lacuerda\.net/([^/]+)/').firstMatch(searchHtml);
          if (videoMatch != null) {
        ...
        for (var i = 0; i < songsList.length; i++) {
          ...
          try {
            ...
            if (artist.startsWith('Letras, Acordes y Tabs') || artist == 'Unknown Artist' || artist.isEmpty) {
              artist = cleanArtist;
            }
            ...
            if (artist == 'LaCuerda') {
              artist = artistQuery.split(' ').map((w) => w.isNotEmpty ? w[0].toUpperCase() + w.substring(1) : '').join(' ');
            }
...
void _updateOrAddCsv(List<Map<String, String>> rows, String title, String artist, String status) {
  var found = false;
  for (final row in rows) {
