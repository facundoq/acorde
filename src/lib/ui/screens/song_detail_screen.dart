...
import '../components/tablature_view.dart';
...
class _SongDetailScreenState extends State<SongDetailScreen> {
  ...
  void _selectScrollSpeed(ScrollSpeed speed) {
    setState(() {
      if (_scrollSpeed == speed) {
        _scrollSpeed = ScrollSpeed.none;
        _stopAutoScroll();
      } else {
        _scrollSpeed = speed;
        int intervalMs = 80;
        if (speed == ScrollSpeed.mid) {
          intervalMs = 40;
        } else if (speed == ScrollSpeed.high) {
          intervalMs = 20;
        }
        _startAutoScroll(intervalMs);
      }
    });
  }
  ...
  Widget _renderStandardContent(String text, ColorScheme colorScheme) {
    final parts = parseGenericTabs(text);

    return Column(
      crossAxisAlignment: CrossAxisAlignment.stretch,
      children: parts.map((part) {
        if (part.type == UGPartType.tablature) {
          return TablatureView(
        ...
        return Column(
          ...
          children: lines.map((line) {
            ...
            for (final match in matches) {
              ...
              final isChord =
                  trimmed.isNotEmpty && chordRegex.hasMatch(trimmed);
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        actions: [
          // Auto scroll buttons (1x, 2x, 3x)
          ...[ScrollSpeed.low, ScrollSpeed.mid, ScrollSpeed.high].map((speed) {
            final isSelected = _scrollSpeed == speed;
            final label = speed == ScrollSpeed.low
                ? '1x'
                : speed == ScrollSpeed.mid
                ? '2x'
                : '3x';
            ...
            return Padding(
              ...
              child: SizedBox(
                height: 28,
                child: TextButton(
                  onPressed: () => _selectScrollSpeed(speed),
